jQuery(document).ready(function($){
    $('body').on('click', '#add-faq:visible', function(e) {
        ...
        newinput.find( '.question' ).attr('name','widget-bttk_faqs_widget['+suffix+'][question]['+len+']');
        newinput.find( '.answer' ).attr('name','widget-bttk_faqs_widget['+suffix+'][answer]['+len+']');
        // newinput.html(function(i, oldHTML) {
    ...
    $(document).on('blur','.user-contact-social-profile',function(e) {
        e.preventDefault();
        $(this).siblings('.bttk-icons-list').fadeOut('slow',function(){
            $(this).remove();
        });
    });
    ...
    $(document).on('click','.bttk-icons-list li',function(event) {
        ...
        $(this).parent().parent().siblings('.bttk-contact-social-length').attr('value','https://'+icon+'.com');
        $(this).parent().parent().siblings('.bttk-social-length').attr('value','https://'+icon+'.com');
    ...
    $(document).on('keyup','.user-contact-social-profile',function() {
        ...
        $(this).siblings('.bttk-icons-list').children('li').show().not(function(){
    ...
    $(document).on('keyup','.search-itw-icons',function() {
        var value = $(this).val();
        var matcher = new RegExp(value, 'gi');
        $(this).siblings('.bttk-font-awesome-list').find('li').show().not(function(){
            return matcher.test($(this).find('svg').attr('data-icon'));
        }).hide();
    });
    ...
    $(document).on('keyup','.bttk-icons-wrap-search',function() {
        var value = $(this).val();
        var matcher = new RegExp(value, 'gi');
        $(this).parent('.bttk-icons-list').children('li').show().not(function(){
            return matcher.test($(this).find('svg').attr('data-icon'));
        }).hide();
    });
    ...
    // ADD IMAGE LINK
        $('body').on('click','.bttk-upload-button',function(e) {
            ...
            .on('select', function() {
                ...
                if( $.inArray( str, strarray ) != -1 ){
                    clicked.find('.bttk-screenshot').empty().hide().append('<img src="' + attachment.url + '"><a class="bttk-remove-image"></a>').slideDown('fast');
                }else{
                    clicked.find('.bttk-screenshot').empty().hide().append('<small>'+bttk_theme_toolkit_pro_uploader.msg+'</small>').slideDown('fast');    
                }
                
                clicked.find('.bttk-upload').val(attachment.id).trigger('change');
                clicked.find('.bttk-upload-button').val(bttk_theme_toolkit_pro_uploader.change);
            }) 
            .open();
        });
        ...
        $('body').on('click','.bttk-remove-image',function(e) {
            ...
            selector.find('.bttk-upload-button').val(bttk_theme_toolkit_pro_uploader.upload);
    ...
    $('body').on('click', '#add-user-socicon', function (e){ 
    	...
    	maximum++;
    	var name = 'widget-bttk_author_bio['+suffix+'][socicon]['+maximum+']';
    	newField.find( '.user-social-links' ).attr('name',name);

        var profile = 'widget-bttk_author_bio['+suffix+'][socicon_profile]['+maximum+']';
    ...
    $('body').on('click', '.bttk-social-add', function(e) {
        ...
        maximum++;
        newinput.find( '.bttk-social-length' ).attr('name','widget-bttk_social_links['+suffix+'][social]['+maximum+']');
        newinput.find( '.user-social-profile' ).attr('name','widget-bttk_social_links['+suffix+'][social_profile]['+maximum+']');
        newinput.html(function(i, oldHTML) {
    ...
    $('body').on('click', '.bttk-contact-social-add:visible', function(e) {
        ...
        maximum++;
        newinput.find( '.bttk-contact-social-length' ).attr('name','widget-bttk_contact_social_links['+suffix+'][social]['+maximum+']');
        newinput.find( '.user-contact-social-profile' ).attr('name','widget-bttk_contact_social_links['+suffix+'][social_profile]['+maximum+']');
        newinput.html(function(i, oldHTML) {
            return oldHTML.replace(/{{ind}}/g, maximum);
        });
        $(this).siblings('.bttk-contact-sortable-links').find('.bttk-contact-social-icon-holder').before(newinput.html()).trigger('change');
    });
    ...
    $('body').on('click', '.bttk-itw-add', function(e) {
        ...
        newinput.html(function(i, oldHTML) {
            ...
            newinput.find( '.text input' ).attr('name','widget-bttk_image_text_widget['+suffix+'][link_text][]');
            newinput.find( '.link input' ).attr('name','widget-bttk_image_text_widget['+suffix+'][link][]');
            newinput.find( '.widget-upload input' ).attr('name','widget-bttk_image_text_widget['+suffix+'][image][]');

            newinput.find( '.text input' ).attr('id','widget-bttk_image_text_widget['+suffix+'][link_text][]');
            newinput.find( '.link input' ).attr('id','widget-bttk_image_text_widget['+suffix+'][link][]');
            newinput.find( '.widget-upload input' ).attr('id','widget-bttk_image_text_widget['+suffix+'][image][]');

            newinput.find( '.text label' ).attr('for','widget-bttk_image_text_widget['+suffix+'][link_text][]');
            newinput.find( '.link label' ).attr('for','widget-bttk_image_text_widget['+suffix+'][link][]');
            newinput.find( '.widget-upload label' ).attr('for','widget-bttk_image_text_widget['+suffix+'][image][]');
        ...
        $(this).siblings('.bttk-img-text-outer').find('.itw-holder').before(newinput.html());
        // $(this).siblings('.bttk-sortable-links').find('.bttk-social-icon-holder').before(newinput.html());
    });
    ...
    $('body').on('click', '.add-logo:visible', function(e) {
        ...
        newinput.html(function(i, oldHTML) {
            newinput.find( '.featured-link' ).attr('name','widget-blossom_client_logo_widget['+suffix+'][link]['+len+']');
            newinput.find( '.widget-upload .link' ).attr('name','widget-blossom_client_logo_widget['+suffix+'][image]['+len+']');
    ...
    $(document).on('click', '.bttk-font-group li', function() {
        ...
        $('#' + id).find('.bttk-font-group li').removeClass();
        $('#' + id).find('.icon-receiver').children('a').remove('.bttk-remove-icon');
        $(this).addClass('selected');
        var prefix =  $(this).parents('.bttk-font-awesome-list').find('.bttk-font-group li.selected').children('svg').attr('data-prefix');
        var icon =  $(this).parents('.bttk-font-awesome-list').find('.bttk-font-group li.selected').children('svg').attr('data-icon');
        var aa = prefix + ' fa-' + icon;
        $(this).parents('.bttk-font-awesome-list').siblings('p').find('.hidden-icon-input').val(aa);
        $(this).parents('.bttk-font-awesome-list').siblings('p').find('.icon-receiver').html('<i class="' + aa + '"></i>');
        $('#' + id).find('.icon-receiver').children('i').after('<a class="bttk-remove-icon"></a>');
    ...
    $(document).on('click', '.bttk-remove-icon', function() {
        ...
        $('#' + id).find('.icon-receiver').html('<i class=""></i>').children('a').remove('.bttk-remove-icon');
        if (in_customizer) {
            $('.hidden-icon-input').trigger('change');
        }
