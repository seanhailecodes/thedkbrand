<?php
/**
 * Blossom Shop Typography Related Functions
 *
 * @package Blossom_Shop
 */

if( ! function_exists( 'blossom_shop_fonts_url' ) ):
/**
 * 
*/ 
function blossom_shop_fonts_url(){
    ...
    $ig_site_title_font = blossom_shop_is_google_font( $site_title_font['font-family'] );
        
    /* Translators: If there are characters in your language that are not
    * supported by respective fonts, translate this to 'off'. Do not translate
    * into your own language.
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'blossom-shop' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'blossom-shop' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'blossom-shop' );
    ...
    if ( 'off' !== $primary || 'off' !== $secondary || 'off' !== $site_title ) {
        ...
        if ( 'off' !== $site_title && $ig_site_title_font ) {
            ...
            if( ! empty( $site_title_font['variant'] ) ){
                $site_title_var = ':' . blossom_shop_check_varient( $site_title_font['font-family'], $site_title_font['variant'] );    
            }else{
                $site_title_var = '';
            }
...
if( ! function_exists( 'blossom_shop_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function blossom_shop_get_google_fonts(){
...
if( ! function_exists( 'blossom_shop_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function blossom_shop_get_websafe_font(){
    ...
    return apply_filters( 'blossom_shop_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'blossom_shop_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function blossom_shop_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = blossom_shop_get_google_fonts(); //Google Fonts
    $websafe_fonts = blossom_shop_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function blossom_shop_get_css_variant( $font_variant ){
    $v_array = array(
        '100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function blossom_shop_is_google_font( $font ){
    $return = false;
    $websafe_fonts = blossom_shop_get_websafe_font();
    if( $font ){
...
*/
function blossom_shop_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = blossom_shop_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( blossom_shop_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = blossom_shop_get_css_variant( blossom_shop_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'blossom_shop_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function blossom_shop_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'blossom_shop_standard_font', array(
        'georgia-serif'       => __( 'Georgia', 'blossom-shop' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'blossom-shop' ),
        'times-serif'         => __( 'Times New Roman, Times', 'blossom-shop' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'blossom-shop' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'blossom-shop' ),
        'comic-cursive'       => __( 'Comic Sans MS, cursive', 'blossom-shop' ),
        'impact-charcoal'     => __( 'Impact, Charcoal', 'blossom-shop' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'blossom-shop' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'blossom-shop' ),
        'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'blossom-shop' ),
        'verdana-geneva'      => __( 'Verdana, Geneva', 'blossom-shop' ),
        'courier'             => __( 'Courier New, Courier', 'blossom-shop' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'blossom-shop' ),
    ) );
